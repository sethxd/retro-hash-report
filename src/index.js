...
async function main() {
  try {
    ...
    if (romFiles.length === 0) {
      ...
      console.log(chalk.dim('Supported extensions: .nes, .sfc, .smc, .gb, .gbc, .gba, .md, .bin, .n64, .iso, and more'));
      process.exit(1);
    }

    console.log(chalk.dim(`Found ${romFiles.length} ROM file(s) in ${romDirectory}\n`));
    ...
    try {
      ...
      spinner.succeed(`Loaded ${consoles.length} systems from RetroAchievements`);
    } catch (error) {
    ...
    try {
      ...
      spinner.succeed(`Loaded ${hashMap.size} known hashes for ${selectedConsole.name}`);
    } catch (error) {
    ...
    spinner.succeed(`Comparison complete: ${matchedCount}/${results.length} matches found`);
    ...
    // Additional info
    if (matchedCount > 0) {
      console.log(chalk.dim('Tip: Matched ROMs have verified achievements on RetroAchievements.org'));
    }
    
    if (results.some(r => !r.match && !r.error)) {
      console.log(chalk.dim('Tip: Unmatched ROMs may be different versions, hacks, or not yet in the RA database'));
    }
