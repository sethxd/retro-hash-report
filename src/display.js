...
export function displayResults(results, options = {}) {
  ...
  console.log(chalk.bold.cyan(`  🎮 RetroAchievements Hash Report - ${consoleName}`));
  console.log(chalk.bold.cyan('━'.repeat(70)));
  console.log('\n');
  ...
  // Summary section
  console.log('\n');
  displaySummary(matched.length, unmatched.length - errors.length, errors.length);
}
...
function displaySummary(matched, unmatched, errors) {
  ...
  console.log(`  ${chalk.green('●')} Matched:   ${chalk.green.bold(matched)} ROMs`);
  console.log(`  ${chalk.yellow('●')} Unmatched: ${chalk.yellow.bold(unmatched)} ROMs`);
  if (errors > 0) {
...
export function displayHeader(text) {
  ...
  console.log(chalk.bold.magenta('║') + chalk.bold.white(centerString(text, 68)) + chalk.bold.magenta('║'));
  console.log(chalk.bold.magenta('╚' + '═'.repeat(68) + '╝'));
  console.log('\n');
}
...
export function displayScanInfo(directory, romCount, consoleName) {
  ...
  console.log(`  ${chalk.bold('ROMs Found:')} ${chalk.cyan(romCount.toString())}`);
  console.log(chalk.dim('─'.repeat(50)));
  console.log('\n');
}
...
export function displayApiStats(gameCount, hashCount) {
  console.log(chalk.dim(`  RetroAchievements: ${gameCount} games, ${hashCount} known hashes`));
}
